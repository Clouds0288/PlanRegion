...
原始标幺约束的联合 SP 容差为 $10^{-8}$，独立 AC 的容差不变。η 是辅助违反量，不是区域距离。方向边界查询使用 0.001 kW 的认证内缩，并与完整 MILP/MISOCP 在 0.002 kW 容差内核对。SOCP 连续域另采用径向精度 tau=0.002；几何容差在公共评价箱归一化坐标中为 1e-8。三维校核网格仅影响 FR/MR 统计。
