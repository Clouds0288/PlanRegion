...
**历史推导：固定方案 SOCP 数学背景。** 本文（包括第 11 节）保留旧算法口径；当前两阶段主线见 [continuous_region.md](continuous_region.md)，不再调用 SP。

**记号作用域：** 本文保留固定树推导的局部符号；当前统一命名见 [数学符号契约](notation.md)。本文 B 是预算、d/q 是标幺负荷、f 是功率因数，A/H/J 是固定树消元矩阵。旧记号换算保留在 [历史契约](notation-v1.md) 第 10 节，不能直接用于当前接口。
