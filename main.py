            ...
            while maximum_answer is not None:
                ...
                for _ in range(REFINEMENT_CHECKS):
                    ...
                    if checked['feasible']:
                        ...
                        if pending is not None and cut[0]+cut[1:1+d]@(pending*bounds/(1-region.tau))+cut[1+d:]@x < -1e-12:
                            pending = None
...
def main():
    ...
    if NETWORK == 'concept5':
        ...
        return run_survey(output=OUTPUT, threads=SOLVER_THREADS, time_limit=CASE_TIME_LIMIT)
