...
def _execute(notebook, function, kwargs, channel):  # spawn 入口必须位于可导入的测试模块。
    try:  # 进程边界统一传回结果或异常。
        ...
        if Path(notebook).suffix == '.py':
            namespace = runpy.run_path(notebook, run_name='_guarded_algorithm')
        else:  # 保留测试自身的 Notebook 夹具；正式算法从 main.py 加载。
            for cell in nbformat.read(notebook,as_version=4).cells:
                tags = cell.metadata.get('tags',[])
                if cell.cell_type=='code' and ('imports' in tags or any(t.endswith('-flow') for t in tags)):
...
def run_guarded(function, kwargs, seconds, *, notebook='main.py'):  # 从测试侧限制一次函数调用的总时间。
