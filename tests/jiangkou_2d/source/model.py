...
from experiment_runtime import optimize
...
class PlanningModel:  # 负责单次规划查询；多次查询和切割循环由 Notebook 组织。
    ...
    def solve(self, time_limit=3600.):  # 返回可行候选和全局界；超时不冒充最优或不可行。
        self.model.Params.TimeLimit = max(0., time_limit)  # 设置本次优化器时限；到时仍需区分证书与未确定状态。
        optimize(self.model, 'planning')  # 调用 Gurobi 求解当前直接模型或含历史割的 MP。
...
class PlanningSP:  # 同一个连续子问题同时支持 LP 和 SOCP 认证及联合割。
    ...
    def solve(self, x, power, time_limit=None):  # 固定整数选型和负荷，时限内返回证书、有效割或未确定结果。
        self.settings.time_limit = 3600. if time_limit is None else max(0.,time_limit)  # 设置单次连续求解时限，数值复核共享这段时间。
...
class RemainingRegionModel:
    ...
    def __init__(self, equations, budget, bounds, total_bound, cuts, inner_halfspaces, tau,
        ...
        for cut in cuts:
            cut = np.asarray(cut)
            scale = max(np.max(np.abs(np.r_[cut[0], cut[1:3]*bounds, cut[3:]])), 1e-20)
        ...
        for k, eq in enumerate(inner_halfspaces):
            ...
            for f, face in enumerate(eq):
                a = face[:2]*(1-tau)
                # z=p/bounds in [0,1]^2；M 由该盒的精确下界推出。
                big_m = 4.-face[2]-np.minimum(a, 0.).sum()
                expression = gp.quicksum(float(a[j]/bounds[j])*problem.power[j].item() for j in range(2))
                m.addConstr(delta <= self.distance_scale*(expression+float(face[2])+float(big_m)*(1-select[f])))
        m.setObjective(delta, GRB.MAXIMIZE)

    def solve(self, tolerance, time_limit=3600.):
