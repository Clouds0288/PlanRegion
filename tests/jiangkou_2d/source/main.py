...
CASE_TIME_LIMIT = 3600.               # 每个预算、每种方法的构域时限（秒）
...
def planning_query(equations, *, power=None, budget=np.inf, direction=None,
    ...
    if np.isinf(budget) and min_total is not None and incumbent is not None:
        # Infinite-budget feasibility needs no additional minimum-cost proof.
        return dict(incumbent, status='feasible_reused_unlimited'), []
    ...
    with problem.model:
        ...
        answer = problem.solve(time_limit=min(3600., max(0., deadline-perf_counter())))
    ...
    if answer['x'] is not None and not answer['feasible'] and perf_counter() < deadline:
        ...
        checked = PlanningSP(equations).solve(answer['x'], answer['p'],
                                            time_limit=min(3600., deadline-perf_counter()))
...
class ContinuousRegion:
    ...
    def check(self, x, z, *, origin='vertex'):
        ...
        checked = self.oracle.solve(x, point, time_limit=self.remaining_time(3600.))
    ...
    def refine(self, x, seed=None, *, witness=None, max_checks=96):
        ...
        for _ in range(max_checks):
            ...
            if checked['feasible']:
                ...
                if pending is not None:
                    cut = checked['cut']
                    if cut[0]+cut[1:3]@(pending*self.bounds/(1-self.tau))+cut[3:]@x < -1e-12:
    ...
    def residual(self):
        ...
        with problem.model:
            answer = problem.solve(GEOMETRY_TOL, time_limit=self.remaining_time(3600.))
        ...
        try:
            ...
            if self.reuse is not None:
                ...
                for certificate in self.reuse['certificates']:
                    ...
                    points = np.asarray(certificate['inner']).reshape(-1, 2)
            ...
            while True:
                ...
                if witness['x'] is None:
                    if self.residual_mode == 'physical' and perf_counter() < self.deadline:
                        self.residual_mode = 'light'
                        self.emit('mode_switch', message='物理搜索暂未取得见证，继承割和证书转轻量搜索')
                        continue
